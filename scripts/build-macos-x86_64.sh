...
cargo build --release --package browser --bin sofintel_helper --target "$target"
...
framework=$(find "$target_dir/build" -path '*/cef_macos_x86_64/Chromium Embedded Framework.framework' -type d -print -quit)
test -n "$framework"
...
hdiutil create -volname Sofintel -srcfolder "$stage" -ov -format UDZO "dist/Sofintel-$version-macos-x86_64.dmg"
rm -rf "$stage"
